...
<main class="min-h-svh bg-[#eee]">
  <div class="px-2 py-4 md:px-4 md:py-6">
    ...
    <div class="max-w-2xl">
      <form action="/dashboard/news" enctype="multipart/form-data" method="POST" class="w-full">
      @csrf
        <div class="mb-4">
          <label for="title" class="block w-fit text-lg font-semibold mb-2">
            Judul :
          </label>
          <input type="text" id="title" name="title" required class="input-title text-lg w-full md:max-w-3xl px-2 py-1 rounded bg-white focus:ring-2 ring-green-400 focus:outline-none @error('title') ring-error @enderror " value="{{old('title')}}">
          @error('title')
          <div>
        ...
        <div class="mb-4">
          ...
          <input type="file" id="img" name="img" required class="input-img bg-slate-300 max-w-xl text-sm px-2 py-1 rounded" value="{{old('img')}}">
          @error('img')
          <div>
            <span class="italic text-red-500">
              {{$message}}
...
<script>
  ...
  inputImg.addEventListener('change', () => {
    ...
    pembacaFile.readAsDataURL(inputImg.files[0]);
    
    pembacaFile.onload = function(targetYangDibaca) {
      previewImg.setAttribute('src', targetYangDibaca.target.result);
    }
  });
  ...
  //   fetch('/dashboard/neew/checkslug?slug=' + inputTitle.value)
