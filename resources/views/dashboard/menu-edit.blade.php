...
<main class="min-h-svh bg-[#eee]">
  <div class="px-2 py-4 md:px-4 md:py-6">
    ...
    <div class="max-w-2xl">
      <form action="/dashboard/menu/{{$menu->slug}}" enctype="multipart/form-data" method="POST" class="w-full">
      @method('put')
      @csrf
        <div class="mb-4">
          ...
          <input type="text" id="name" name="name" required class="inputName text-lg max-w-2xl px-2 py-1 rounded bg-white focus:ring-2 ring-green-400 focus:outline-none" value="{{old('name') ? old('name') : $menu->name }}">
        </div>
        {{-- <div class="mb-4">
          <label for="name" class="block w-fit text-lg font-semibold mb-2">
            Slug :
        ...
        <div class="mb-4">
          ...
          <select id="discount" name="is_diskon" required class="text-lg px-2 py-1 rounded bg-white focus:outline-none">
            ...
            @elseif($menu->is_diskon == 0)
            <option value="0" class="border-0" selected>No Discount</option> 
            <option value="1" class="border-0">Discount</option> 
            @elseif($menu->is_diskon == 1)
            <option value="0" class="border-0">No Discount</option> 
            <option value="1" class="border-0" selected>Discount</option> 
            @endif
          </select>
        ...
        <div class="mb-4">
          ...
          <input type="file" id="img" name="img" class="input-img bg-slate-300 max-w-xl text-sm px-2 py-1 rounded" value="{{$menu->img}}">
         ...
         @error('img')
          ...
          @enderror
          @if($menu->img)
          <img src="{{asset('storage/' . $menu->img )}}" alt="{{}}" class="preview-img w-24 h-24 mt-4 object-center object-cover">
          @endif
        </div>
        <button type="submit" class="text-white text-lg font-semibold bg-green-500 rounded px-2 py-1">
          Simpan
        </button>
      </form>
    </div>
